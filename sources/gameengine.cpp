    ...
    : m_mainWindow(mainWindow), randomCounter(0)
{
    ...
    m_cheatsEnabled = !qgetenv("DEBUG").isEmpty();
...
{
    if (gameIsPaused()) {
        qDebug() << "trying to fire while game is paused!!!";
        resume();
        return;
    }
...
void GameEngine::changeSpeed(qreal ratio) {
    qDebug() << "Update interval =" << m_gameTimer.interval();
    m_speed *= ratio;
    if (m_speed > 2.0) {
...
void GameEngine::loadNextLevel()
{
    ...
    foreach (Brick *brick, m_bricks) {
        ...
        if (brick->identifier() == "UnbreakableBrick") {
            continue;
        }
        if (brick->isDeleted()) {
            continue;
