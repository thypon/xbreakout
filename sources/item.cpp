...
// static
CanvasWidget *Item::canvas = 0;
qreal Item::m_scale = 1.0;
int Item::m_borderLeft = 0;
int Item::m_borderTop = 0;

// static
void Item::setCanvas(CanvasWidget *c)
{
    canvas = c;
}

Item::Item(QString id, int w, int h) 
    : QGraphicsPixmapItem(),
      elementId(id),
      width(w),
      height(h)
{
    if (canvas == 0) {
        qDebug() << "Item::Item(): a scene must be set "
...
void Item::updateScale()
{
    ...
    m_borderTop = qRound( (static_cast<qreal>(canvas->height()) 
                       - m_scale * (BRICK_HEIGHT * (HEIGHT-2))) / 2 );
}
...
void Item::setIdentifier(const QString &type)
{
    elementId = type;
    loadSprite();
}
...
{
    qDebug() << "setRect() does nothing, change it!\n";
